...
"""
Author: Lori Garzio on 7/13/2020
...
def add_slp_contours(ax, londata, latdata, slp_data):
    contour_list = np.round(np.linspace(994, 1024, 9))
...
def plt_heatflux(nc, varname, model, ax_lims, save_dir):
    ...
    for i, t in enumerate(nc['Time'].values):
        ...
        figttl = '{}: {} UTC'.format(var, pd.to_datetime(t).strftime('%b %d %Y %H:%M'))
...
def plt_slp(nc, model, ax_lims, save_dir):
    ...
    levels = np.round(np.linspace(994, 1024, 9))
    #levels = np.round(np.linspace(994, 1024, 12))
...
def plt_sst(nc, model, ax_lims, save_dir):
    ...
    for i, t in enumerate(nc['Time'].values):
        ...
        figttl = 'Sea Surface Temperature: {} UTC'.format(pd.to_datetime(t).strftime('%b %d %Y %H:%M'))

        fig, ax = plot_pcolor(fig, ax, figttl, lon, lat, sst_sub_c, 0, 40, 'jet', color_label)

        add_map_features(ax, ax_lims, 'darkgray')
        #add_map_features(ax, ax_lims)
...
def plt_windsp(nc, model, ht, ax_lims, save_dir):
    ...
    for i, t in enumerate(nc['Time'].values):
        ...
        figttl = '{}m Wind Speeds: {} UTC'.format(ht, pd.to_datetime(t).strftime('%b %d %Y %H:%M'))

        fig, ax = plot_pcolor(fig, ax, figttl, lon, lat, speed.values, 0, 20, 'BuPu', color_label)
...
def main(f):
    ...
    for pv in plt_vars:
        if pv == 'U10':
            plt_windsp(ncfile, 'fay_wrf_his', 10, axlims, sDir)
        elif pv == 'SST':
            plt_sst(ncfile, 'fay_wrf_his', axlims, sDir)
        elif pv == 'SLP':
            plt_slp(ncfile, 'fay_wrf_his', axlims, sDir)
        elif pv in ['LH', 'HFX']:
            plt_heatflux(ncfile, pv, 'fay_wrf_his', axlims, sDir)


if __name__ == '__main__':
    fpath = '/Users/lgarzio/Documents/rucool/Miles/NOAA_COMT/tsfay/20200715/fay_wrf_his_d01_2020-07-09_12_00_00_subset.nc'  # on local machine
    # fpath = '/home/lgarzio/rucool/NOAA_COMT/tsfay/fay_wrf_his_d01_2020-07-09_00_00_00.nc'  # on server
    main(fpath)
